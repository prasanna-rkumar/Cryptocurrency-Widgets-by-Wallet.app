...
class Wallet_App_Currency_Widget extends WP_Widget {
	...
	public function form( $instance ) {
		...
		$defaults = array(
			...
			'bgcolor'     		=> '',
			'width'       		=> '0', 
			'coinID' 			=> 'bitcoin,ethereum,ripple,eos,litecoin,bitcoin-cash,nem',
			'currencyCode'   	=> '',
		);
		...
		<?php
			/* $toDisplay = array('bitcoin', 'ethereum', 'ripple', 'eos', 'nem', 'bitcoin-gold');
			$curr = explode(',', $coinID);

			for($i = 0; $i < sizeof($curr); $i++)
			{
				if( ! (in_array($curr[$i], $toDisplay)) )
				{
					array_push($toDisplay, $curr[$i]);
				}
			}

			$coinID = implode(',', $toDisplay); */
		?>
		...
		<p>
			...
			<div id="<?php echo $this->get_field_name('coinIDList') ?>">
				<?php 
					/* $curr = explode(',', $coinID);
					for($i = 0; $i < sizeof($curr); $i++)
					{
						...
						echo '<span data-coin-id="${coins[i]}" class="dashicons dashicons-no-alt"></span>';
					} */
				?>
			</div>
		</p>
		...
		<p>
			...
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'coinID' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'coinID' ) ); ?>" type="text" value="<?php echo esc_attr( $coinID ); ?>" />
		</p>
		...
		<p>
			<input id="currencyWidgetSearchInput" class="currency-search-input" name="walletAppCurrencySearch" list="currencies"> 
			<input type="button" value="Add" />
			<datalist id="currencies">
				<?php 
					for($i = 0; $i < sizeof($body); $i++)
					{
						echo '<option value="' . esc_attr( $body[$i]['id'] ) . '">';
					}
				?>
			</datalist>
		</p>
	...
	public function widget( $args, $instance ) {
		...
		echo '<'.$widget_marquee.'  coin-ids="'.$coinID.'" currency="'.$currencyCode.'" locale="en" background="'.$bgcolor.'"'.$$widthText.'"/>';
